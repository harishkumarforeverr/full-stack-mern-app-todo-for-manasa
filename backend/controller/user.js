...
const sortByPriory = async (req, res) => {
  console.log(11111111);
  try {
    const users = await Person.find({}).sort({ Priority: -1 });
    res.send(users);
  } catch (err) {
    res.send("Something went wrong,  plz try agisssssn");
  }
};

const sortByDate = async (req, res) => {
  try {
    const users = await Person.find({}).sort({ Date: 1 });
    res.send(users);
  } catch (err) {
    res.send("Something went wrong,  plz try agisssssn");
  }
